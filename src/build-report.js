...
<div class="container">
  <header>
    ...
    <p class="subtitle">Generated by Fractionator</p>
  </header>

  ${hasAlignment ? `<div class="tab-bar" role="tablist">
  ...
  ${hasAlignment ? `<div class="tab-panel hidden" id="tab-alignment">
    ${alignmentSection}
  </div>` : ""}
</div>
...
function renderParam(p) {
  const binding = p.isBinding ? '<span class="annotation">@Binding</span> ' : "";
...
function renderMarkdown(catalogue) {
  ...
  for (const [platform, data] of Object.entries(catalogue.platforms)) {
    ...
    lines.push(
      `| Component | Usages | Variants | Used in |`,
    );
    lines.push(`|-----------|--------|----------|---------|`);
    ...
    for (const comp of sorted) {
      const screens = [...new Set(comp.usages.map((u) => u.enclosingView))].join(", ");
      lines.push(
