...
| [design-system-package-input.md](design-system-package-input.md) | Proposed | Make an importable design-system **Swift Package** a first-class input (not just an app prototype): detect `Package.swift` + a library mode, discover public components without a `Components/` dir, stop excluding them as "unused", and read tokens from definitions — colorsets + `Color(…, bundle:)` aliases, the `Font.custom` ramp, and enum spacing/stroke/corner-radius scales. Fixes the "no results" when scanning the NHS iOS design-system package. |
